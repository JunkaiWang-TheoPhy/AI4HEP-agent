...
_LOADED_BLOCK_IDS: set[str] = set()          # which blocks have been exec()'d
...
# DO NOT change these two lines; they re-lock the markers at runtime even if a generator corrupts code lines
...
def _normalize_marker_lines(text: str) -> Tuple[str, bool]:
    ...
    changed = False
    # Any line that contains the marker becomes canonical (strip stray quotes, tail)
...
def _ensure_dynamic_region_exists(text: str) -> Tuple[str, bool]:
    ...
    if has_start and has_end:
        # It exists; ensure it's before entry; else move.
        ...
        if start < insert_idx:
            return text, normed
        # Move the whole region before ENTRY
...
def _exec_dynamic_code(code: str, block_id: str) -> List[str]:
    ...
    g = {"mcp": mcp, "__name__": f"dyn_{block_id}"}
    l: Dict[str, Any] = {}
    exec(compiled, g, l)
...
def repair_self_file(
    ...
    # 3) Normalize marker lines again (in case move created dup newlines)
    text, normed = _normalize_marker_lines(text)
    changed2 = changed2 or normed
    reg_start, reg_end, body_start, body_end = _find_dynamic_region(text)

    # 4) Move orphan blocks inside region
    moved, ignored = [], []
    if migrate_orphans:
        all_blocks = _scan_all_blocks(text)
        region_blocks = _parse_blocks(text[body_start:body_end])
        # Walk from end to start so indices remain valid when removing
        for start, end, bid, body in reversed(all_blocks):
            inside = (start >= body_start and end <= body_end)
            if inside:
                continue
            # outside region -> orphan
            if bid in region_blocks:
                if prefer == "orphan":
                    # replace region copy with the orphan body
                    pattern = re.compile(
                    ...
                    # remove orphan occurrence
                    text = _remove_span(text, start, end)
                    changed2 = True
                    moved.append(bid)
                else:
                    # keep region copy; drop orphan
                    text = _remove_span(text, start, end)
                    changed2 = True
                    ignored.append(bid)
            else:
                # Append orphan at end of region
                region_text = text[body_start:body_end]
                if region_text and not region_text.endswith("\n"):
                    region_text += "\n"
                region_text += _render_block(bid, body)
                text = text[:body_start] + region_text + text[body_end:]
                # remove original orphan
                # Need to recompute orphan indices due to insert; best-effort: find again and remove first occurrence outside region
                # Quick approach: remove by slicing previous range (which is still valid because we appended after body_end)
                text = _remove_span(text, start, end)
                changed2 = True
                moved.append(bid)
    ...
    # 5) Clean accidental trailing quotes on marker lines (very specific corruption)
    # e.g. '# === DYNAMIC TOOLS END ==="'
